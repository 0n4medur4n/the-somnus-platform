...
const prepColors = (input?: string[]) => {
  const base = (input?.length ? input : ["#9FC0FF", "#437EF7", "#1746A2"]).slice(0, MAX_COLORS);
  const count = base.length;
  const arr: RGB[] = [];
  const fallbackColor = base[base.length - 1] ?? "#437EF7";
  for (let i = 0; i < MAX_COLORS; i++) {
    arr.push(hexToRGB(base[Math.min(i, base.length - 1)] ?? fallbackColor));
  }
  const avg: RGB = [0, 0, 0];
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      if (mouseDampening > 0) {
        ...
        const currentX = cur[0] ?? 0;
        const currentY = cur[1] ?? 0;
        cur[0] = currentX + (target[0] - currentX) * factor;
        cur[1] = currentY + (target[1] - currentY) * factor;
      } else {
        lastTimeRef.current = t;
      }
