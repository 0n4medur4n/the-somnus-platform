...
export class SomnusExceptionFilter implements ExceptionFilter {
  private readonly nestLogger = new NestLogger(SomnusExceptionFilter.name);
    ...
    // Fastify-plugin errors (e.g. @fastify/rate-limit's 429,
    // @fastify/csrf-protection's 403) arrive as plain errors carrying a
    // numeric `statusCode`. Map that to the §16 shape too, so a rate
    // limit reads as RATE_LIMITED, not a generic INTERNAL 500.
...
function extractStatusCode(exception: unknown): number | null {
