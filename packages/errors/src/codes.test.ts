...
describe("errorCodeToHttpStatus", () => {
  it("maps every known code to a 4xx or 5xx status", () => {
    const knownCodes: ReadonlyArray<ErrorCodeType> = Object.values(ErrorCode) as ErrorCodeType[];
  ...
  it("rejects an unknown code via runtime check (no silent fallthrough)", () => {
    const status = (errorCodeToHttpStatus as Record<string, number | undefined>)[
      "__NOT_A_REAL_CODE__"
    ];
